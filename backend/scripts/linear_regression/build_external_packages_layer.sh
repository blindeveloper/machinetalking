...
file_name="lin_reg_external_pkgs_layer_${current_time_ms}.zip"
...
docker run --rm --platform linux/arm64 -v $(pwd):/foo -w /foo public.ecr.aws/sam/build-python3.9:latest \
    ...
    pip3 install -r external_pkgs_requirements.txt --index-url https://download.pytorch.org/whl/cpu --no-deps --no-cache-dir -t ${PKG_DIR}
